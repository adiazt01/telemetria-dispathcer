...
void imprimirUso(const char* programa) {
    printf("========================================\n");
    printf("  SENSOR - Modulo 1 (Carlos Brito)\n");
    printf("========================================\n");
    printf("\n");
    printf("Uso: %s <id_sensor> <tipo_sensor>\n", programa);
    printf("\n");
    printf("  id_sensor:    Identificador unico del sensor (0-255)\n");
    printf("  tipo_sensor:  Tipo de sensor a simular:\n");
    printf("                0 = Motor\n");
    printf("                1 = Neumaticos\n");
    printf("                2 = Frenos\n");
    printf("                3 = GPS\n");
    printf("\n");
    printf("Ejemplo: %s 1 0\n", programa);
    printf("         Crea un sensor de MOTOR con ID=1\n");
    printf("\n");
}
...
BOOL conectarAlBroker() {
    ...
    while (g_running) {
        g_hPipe = CreateFile(
            g_pipeName,                     /* Nombre del pipe */
            GENERIC_READ | GENERIC_WRITE,  /* Lectura y escritura */
            0,                              /* No compartido */
            NULL,                           /* Atributos de seguridad default */
            OPEN_EXISTING,                  /* Solo si existe */
            FILE_FLAG_OVERLAPPED,          /* Modo overlapped */
            NULL                            /* Template file (ninguno) */
        );

        if (g_hPipe != INVALID_HANDLE_VALUE) {
            printf("[Sensor %d] Conectado al Broker exitosamente!\n", g_sensorId);
            return TRUE;
        }

        /* Si no existe el pipe, esperamos un poco y reintentamos */
        if (GetLastError() != ERROR_FILE_NOT_FOUND) {
            printf("[Sensor %d] Error al conectar: %d\n", g_sensorId, GetLastError());
            Sleep(100);
        }
...
BOOL enviarEvento(const SensorEvent* evento) {
    ...
    resultado = WriteFile(
        g_hPipe,                    /* Handle al pipe */
        evento,                     /* Buffer con el evento */
        sizeof(SensorEvent),        /* Tamano del evento */
        &bytesWritten,              /* Bytes escritos (output) */
        NULL                        /* No overlapped */
    );

    if (!resultado || bytesWritten != sizeof(SensorEvent)) {
...
void generarEvento(SensorEvent* evento) {
    ...
    evento->sensorId = g_sensorId;
    evento->sensorType = g_sensorType;
    ...
    switch (g_sensorType) {
        case SENSOR_MOTOR:
        case SENSOR_FRENOS:
            evento->priority = PRIORITY_HIGH;
            break;
        case SENSOR_NEMATICOS:
            evento->priority = PRIORITY_NORMAL;
            break;
        case SENSOR_GPS:
            evento->priority = PRIORITY_LOW;
            break;
        default:
            evento->priority = PRIORITY_NORMAL;
    }
    ...
    evento->payloadSize = MAX_PAYLOAD_SIZE;
    generarPayload(evento->payload, evento->payloadSize, g_sensorType);
}
...
BOOL controlarTecla(DWORD waitTime) {
    ...
    if (WaitForSingleObject(GetStdHandle(STD_INPUT_HANDLE), waitTime) == WAIT_OBJECT_0) {
        ...
        if (input.EventType == KEY_EVENT && input.KeyEvent.bKeyDown) {
            /* ESC para salir */
            if (input.KeyEvent.wVirtualKeyCode == VK_ESCAPE) {
...
int main(int argc, char* argv[]) {
    ...
    printf("========================================\n");
    printf("  SENSOR - Modulo 1\n");
    printf("  Responsabilidad: Carlos Brito\n");
    printf("========================================\n\n");
    ...
    g_sensorId = atoi(argv[1]);
    if (g_sensorId > 255) {
        printf("Error: ID del sensor debe ser 0-255\n");
        return 1;
    }

    /* Parsear tipo de sensor */
    g_sensorType = atoi(argv[2]);
    if (g_sensorType < 0 || g_sensorType > 3) {
        printf("Error: Tipo de sensor debe ser 0-3\n");
        return 1;
    }

    /* Construir nombre del pipe para este sensor */
    snprintf(g_pipeName, MAX_PATH, PIPE_NAME_FORMAT, g_sensorId);

    printf("[Sensor %d] Tipo: %s\n", g_sensorId, obtenerNombreSensor(g_sensorType));
    printf("[Sensor %d] Pipe: %s\n", g_sensorId, g_pipeName);
    ...
    /* Conectar al Broker */
    if (!conectarAlBroker()) {
        printf("[Sensor %d] No se pudo conectar al Broker\n", g_sensorId);
        return 1;
    }
    ...
    printf("[Sensor %d] Presione ESC para detener\n", g_sensorId);
    ...
    while (g_running) {
        ...
        /* Tiempo aleatorio entre envios (100-500ms) para simular
           diferentes frecuencias de muestreo de sensores reales */
        sleepTime = 100 + rand() % 400;

        /* Verificar si se presiono ESC mientras dormimos */
        if (!controlarTecla(sleepTime)) {
            break;
        }
    }
